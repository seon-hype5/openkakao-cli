...
> 합성 테스트, 리덕션된 `doctor --ui` 진단 경로는 준비되어 있습니다. 기본
> 빌드에서는 UI 쓰기 코드가 제외됩니다. 기본 비활성 기능 플래그 빌드에는
> 실험적인 프로필 v3 나와의 채팅 전송 경로가 있지만 전체 실측 활성화 기준을
> 아직 통과하지 않았으므로 일반 사용 기능으로 보장하지 않습니다. 이 경로는
> 카카오톡 데이터베이스·자격 증명·클립보드·전역 키 입력을 사용하지 않으며,
> 전송 직전에 검증된 카카오톡 창과 작성창만 포커스합니다.
> 자세한 상태와 안전 경계는
